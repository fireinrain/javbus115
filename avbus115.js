...
// @downloadURL https://update.sleazyfork.org/scripts/480566/JAV%E8%80%81%E5%8F%B8%E6%9C%BA%EF%BC%8CJAV%E5%90%84%E5%A4%A7%E7%BD%91%E7%AB%99%E4%BC%98%E5%8C%96%E3%80%9018%E5%B2%81%E4%BB%A5%E4%B8%8B%E7%94%A8%E6%88%B7%E8%AF%B7%E5%8B%BF%E5%AE%89%E8%A3%85%E3%80%91.user.js
// @updateURL https://update.sleazyfork.org/scripts/480566/JAV%E8%80%81%E5%8F%B8%E6%9C%BA%EF%BC%8CJAV%E5%90%84%E5%A4%A7%E7%BD%91%E7%AB%99%E4%BC%98%E5%8C%96%E3%80%9018%E5%B2%81%E4%BB%A5%E4%B8%8B%E7%94%A8%E6%88%B7%E8%AF%B7%E5%8B%BF%E5%AE%89%E8%A3%85%E3%80%91.meta.js
// ==/UserScript==
/* jshint -W097 */
(function () {
    'use strict';
    ...
    messageContainer.id = 'tm-message-container';
    document.body.appendChild(messageContainer);

    // 定义显示消息的函数
    function showMessage(message, isSuccess) {
        const container = document.getElementById('tm-message-container');
        container.textContent = message;
        container.className = isSuccess ? 'tm-success' : 'tm-error';
    ...
    // 将showMessage函数添加到全局作用域，以便其他脚本可以使用
            ...
            w.setThirdCallback(function (elems) {
                ...
                if (document.title.search(/JAVLibrary/) > 0 && elems) {
                    for (let i = 0; i < elems.length; i++) {
                        ...
                        let prom = javDb.select().from(myMovie).where(lf.op.and(myMovie.is_browse.eq(true), myMovie.index_cd.eq(_vid))).exec();
                        prom.then(results => {
    ...
    //按照时间日期重新排序列表
    function sortItemWithDateButton() {
        let a3 = document.createElement('a');
        let s = GM_getValue('scroll_status', 0)
        if (s <= 0) {
            return null;
        }
        $(a3).append('开启日期排序&nbsp;&nbsp;')
        ...
        $(a3).attr("href", "#");
        $(a3).click(function () {
            console.log("按日期进行重排序...")
            // 获取包含所有 .item 的容器
            const container = document.getElementById('waterfall_h');
            ...
            // 对 items 进行排序
            items.sort((a, b) => {
                const dateA = new Date(a.querySelector('date:nth-of-type(2)').textContent);
                const dateB = new Date(b.querySelector('date:nth-of-type(2)').textContent);
    ...
    function javBusScript() {
        ...
        if ((/(JavBus|AVMOO|AVSOX)/g).test(document.title) || $("footer:contains('JavBus')").length) {
            ...
            if (a4 !== null) {
                ...
                $(li_elem2).append($(a4));
                // JavBus
                $(".visible-md-block").closest(".dropdown").after($(li_elem2));
                // AVMOO|AVSOX
                $(".active").closest(".navbar-nav").append($(li_elem2));
            }
...
//如果离线下载控制台出现: 你的帐号使用异常，需要在线手工重新验证即可正常使用
//你只需要 在115网页端 新建一个离线 这时候会弹出一个验证 过了就好了